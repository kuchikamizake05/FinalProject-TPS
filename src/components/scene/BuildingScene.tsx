...
function SideColumn({ x, label }: { x: number; label?: string }) {
  ...
  const innerPanelX = x > 0 ? -0.325 : 0.325
  const innerWhiteX = x > 0 ? -0.36 : 0.36
  const innerFrameX = innerPanelX + (x > 0 ? 0.02 : -0.02)

  const outerPanelX = x > 0 ? 0.325 : -0.325
  const outerFrameX = outerPanelX + (x > 0 ? 0.02 : -0.02)
  ...
  return (
    <group position={[x, 2.8, buildingCenterZ]}>
      ...
      <group>
        {/* 1. INNER SIDE (towards interior): Blue panel with dense horizontal louvre slats */}
        <mesh position={[innerPanelX, panelY, 0]} rotation={[0, x > 0 ? Math.PI / 2 : -Math.PI / 2, 0]} castShadow receiveShadow>
          <boxGeometry args={[buildingDepth, panelHeight, thickness]} />
          <meshStandardMaterial color="#1e3a8a" roughness={0.26} metalness={0.3} />
        </mesh>
        
        {/* Dense horizontal louvre slats overlay (Inner Side) */}
        {Array.from({ length: 80 }).map((_, sIdx) => {
          const slatLocalY = (sIdx / 79) * panelHeight - (panelHeight / 2) + panelY
          return (
            <mesh key={`side-slat-${sIdx}`} position={[innerFrameX, slatLocalY, 0]} rotation={[0, x > 0 ? Math.PI / 2 : -Math.PI / 2, 0]} castShadow>
        ...
        {/* Interior white panel for the inner side */}
        <mesh position={[innerWhiteX, panelY, 0]} rotation={[0, x > 0 ? Math.PI / 2 : -Math.PI / 2, 0]} castShadow receiveShadow>
          <boxGeometry args={[buildingDepth, panelHeight, thickness]} />
          <meshStandardMaterial color="#ffffff" roughness={0.48} metalness={0.1} />
        </mesh>

        {/* 2. OUTER SIDE (towards outside edge): Beautiful horizontal window column */}
        {/* Bright sky-blue reflective glass pane */}
        <mesh position={[outerPanelX, panelY, 0]} rotation={[0, x > 0 ? Math.PI / 2 : -Math.PI / 2, 0]} castShadow receiveShadow>
          <boxGeometry args={[buildingDepth, panelHeight, thickness]} />
          <meshStandardMaterial color="#0ea5e9" roughness={0.05} metalness={0.9} transparent opacity={0.85} />
        </mesh>

        {/* Front and back vertical white frames for the window column */}
        {[-buildingDepth / 2, buildingDepth / 2].map((zVal, zIdx) => (
          <mesh key={`win-frame-vert-${zIdx}`} position={[outerFrameX, panelY, zVal]} rotation={[0, x > 0 ? Math.PI / 2 : -Math.PI / 2, 0]} castShadow>
            <boxGeometry args={[0.03, panelHeight, 0.03]} />
            <meshStandardMaterial color="#ffffff" roughness={0.4} />
          </mesh>
        ))}

        {/* Horizontal frames & transoms for the window column */}
        {Array.from({ length: 11 }).map((_, f) => {
          const localY = f * floorStep - 2.8
          if (localY < -2.8 || localY > 3.52) return null
          return (
            <group key={`win-frame-floor-${f}`} position={[outerFrameX + (x > 0 ? 0.001 : -0.001), localY, 0]} rotation={[0, x > 0 ? Math.PI / 2 : -Math.PI / 2, 0]}>
              {/* Floor divider */}
              <mesh castShadow>
